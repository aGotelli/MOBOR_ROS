...
void poseCallback(const nav_msgs::OdometryConstPtr& targetOdometry)
{
  ...
  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "map", ros::this_node::getName()));
}
...
int main (int argc, char** argv)
{
	...
	//ROS Initialization
  ros::init(argc, argv, "CardroidTarget");
  ...
  double rInt;
  targetNode.param("inner_radius", rInt, 1.0);


  double rExt;
  targetNode.param("outter_radius", rExt, 2.0);
  ...
  while(ros::ok()) {
        ...
        targetOdometry.pose.pose.position.x = (rInt + rExt*cos(omega*t))*cos(omega*t);
        targetOdometry.pose.pose.position.y = (rInt + rExt*cos(omega*t))*sin(omega*t);
        ...
        targetOdometry.twist.twist.linear.x = -rInt*sin(omega*t) - rExt*sin(2*omega*t);
        targetOdometry.twist.twist.linear.y = rInt*cos(omega*t) + rExt*cos(2*omega*t);

        targetPublisher.publish(targetOdometry);

        sim::SetCoordinates(Pose(targetOdometry.pose.pose.position.y, targetOdometry.pose.pose.position.y, 1), targetMarker);
